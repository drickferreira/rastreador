...
class LoadCRXPositions extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'positions:CRX';
     ...
     * @var string
     */
    protected $description = 'Acessa o Gateway CRX e atualiza as posicoes transmitidas';
							...
							try {
								...
								foreach($xml->xpath('POSITION') as $pos){
										$device = Device::where('serial', xmlGetVal($pos,'FIRMWARE/SERIAL'))
																			->where('model', $model)
																			->first();
											if (xmlGetVal($pos,'GPS/LATITUDE','str') == '') continue;
